...
历史按只追加处理：保存每次扫描结束的字节位置和分页模式，下次直接跳过旧内容，
只扫描新增记录，不再读取旧前缀或计算整文件哈希。旧版 `.sanitized` 缓存的位置可直接沿用。
文件未变化时只检查元数据；文件替换、缩短、同大小修改、缓存缺失或末行不完整时重新扫描。
同一文件原地改写旧内容并增长不属于只追加假设。发现需要清理的新记录时仍在写锁下完整原子重写，
保留历史序号和字节偏移。首次无缓存扫描仍与历史大小相关，后续扫描只与新增内容大小相关。
...
printf '%s\n' '{"openai":{"default":"gpt-6-astra","models":["gpt-6-astra","gpt-5.6-sol","gpt-5.6-terra","gpt-5.6-luna","gpt-5.5","gpt-5.4","gpt-5.4-mini","gpt-5.3-codex-spark"]},"sub2api":"gpt-5.6-sol","glm":{"default":"glm-5.3","models":["glm-5.3","glm-5.3-flash","glm-5.2"]},"kimi":"k3","deepseek":{"default":"deepseek-v4-flash","models":["deepseek-v4-flash","deepseek-v4-pro"]}}' \
  > "$MODELS_STAGE"
