...
Durable provider-and-model selections describe the desired route, not the loaded
runtime. Ordinary turns and explicit switches compare against the observed route,
using the session cache when available and `thread/list` discovery otherwise.
