...
`internal/modelroute` loads the optional strict `models.json` provider-to-model
catalog from the switcher state directory. `internal/rewrite` validates one
...
model untouched. Ordinary prompts that merely mention `/provider` or
