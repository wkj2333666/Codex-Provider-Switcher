...
The first implementation targets Go 1.24 and Linux and macOS on amd64 and
arm64. Unix-domain socket validation makes native Windows out of scope.
...
four cross-builds. The project does not modify a user's system.

## Release Automation

Pushing a tag matching `v*` runs a dedicated GitHub Actions workflow. The
workflow validates the tag, runs the same quality gates as CI, and builds the
following `CGO_ENABLED=0` binaries with the tag injected into `--version`:
...
Each target is packaged as a versioned `.tar.gz` archive containing the binary,
README, and license. A matching `.sha256` file is generated for every archive.
Artifacts are collected by a separate publish job that alone receives
`contents: write`; it creates a GitHub Release with generated release notes.
...
- A `v*` tag creates a GitHub Release containing four versioned archives and
